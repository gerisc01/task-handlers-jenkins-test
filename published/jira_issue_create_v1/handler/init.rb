...
class JiraIssueCreateV1
  ...
  def execute()
    ...
    body = {"fields"=>{
          ...
          "issuetype" => {"name" => @parameters['type']},
          "description" => @parameters['description'],
          "assignee" => {"name" => @parameters['assignee']},
          "priority" => {"name" => @parameters['priority']},
          "description" => @parameters['description']
        }}
...
are being sent over an insecure connection if ssl is not configured."
    ...
    resp = resource['rest/api/2/issue'].post data, :content_type => "application/json"
    ...
    <<-RESULTS
    <results>
      <result name="id">#{results['id']}</result>
      <result name="key">#{results['key']}</result>
      <result name="url">#{results['self']}</result>
    </results>
    RESULTS
  end
